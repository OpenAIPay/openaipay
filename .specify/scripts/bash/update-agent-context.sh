...
extract_plan_field() {
    ...
    local plan_file="$2"
    local candidate_labels=("$field_pattern")
    local candidate
    local raw_value=""

    case "$field_pattern" in
        "Language/Version")
            candidate_labels+=("语言/版本")
            ;;
        "Primary Dependencies")
            candidate_labels+=("核心依赖")
            ;;
        "Storage")
            candidate_labels+=("存储方式")
            ;;
        "Testing")
            candidate_labels+=("测试方案")
            ;;
        "Target Platform")
            candidate_labels+=("目标平台")
            ;;
        "Project Type")
            candidate_labels+=("项目类型")
            ;;
        "Performance Goals")
            candidate_labels+=("性能目标")
            ;;
        "Constraints")
            candidate_labels+=("约束条件")
            ;;
        "Scale/Scope")
            candidate_labels+=("规模/范围")
            ;;
    esac

    for candidate in "${candidate_labels[@]}"; do
        raw_value=$(grep "^\*\*${candidate}\*\*: " "$plan_file" 2>/dev/null | \
            head -1 | \
            sed "s|^\*\*${candidate}\*\*: ||" | \
            sed 's/^[ \t]*//;s/[ \t]*$//') || true
        if [[ -n "$raw_value" ]]; then
    ...
    printf '%s\n' "$raw_value" | grep -v "NEEDS CLARIFICATION" | grep -v "^N/A$" || echo ""
}
...
get_project_structure() {
    ...
    if [[ "$project_type" == *"mobile-app"* ]] && [[ "$project_type" == *"web-service"* ]]; then
        echo "backend/\\nios-app/\\napp-bff/\\nlocal/\\nspecs/"
...
get_language_conventions() {
    ...
    if [[ "$lang" == *"Swift"* ]] && [[ "$lang" == *"Java"* ]]; then
        echo "Swift：遵循 SwiftUI 的视图、状态与功能目录约定；Java：遵循现有 Spring Boot 分层、MyBatis-Plus 持久化和 Flyway 迁移约定。"
        return
    fi

    echo "$lang：遵循仓库中的既有约定。"
}
...
create_new_agent_file() {
    ...
    if [[ -n "$escaped_db" ]] && [[ "$escaped_db" != "N/A" ]] && [[ "$escaped_db" != "NEEDS CLARIFICATION" ]]; then
        tech_stack="${tech_stack}\\n- $escaped_db ($escaped_branch)"
    fi
    ...
    if [[ -n "$escaped_lang" && -n "$escaped_framework" ]]; then
        ...
        recent_change="- $escaped_branch: 新增规划上下文"
    fi
    ...
    local substitutions=(
        ...
        "s|\[从所有 PLAN.MD 中提取的技术栈\]|$tech_stack|"
        "s|\[ACTUAL STRUCTURE FROM PLANS\]|$project_structure|g"
        "s|\[根据计划汇总出的实际项目结构\]|$project_structure|g"
        "s|\[ONLY COMMANDS FOR ACTIVE TECHNOLOGIES\]|$commands|"
        "s|\[仅保留当前技术栈实际需要的命令\]|$commands|"
        "s|\[LANGUAGE-SPECIFIC, ONLY FOR LANGUAGES IN USE\]|$language_conventions|"
        "s|\[仅保留当前已使用语言对应的代码风格约定\]|$language_conventions|"
        "s|\[LAST 3 FEATURES AND WHAT THEY ADDED\]|$recent_change|"
        "s|\[最近 3 个功能及其新增内容\]|$recent_change|"
    )
...
update_existing_agent_file() {
    ...
    local has_current_branch_change=0

    if grep -Fq -- "$CURRENT_BRANCH:" "$target_file" 2>/dev/null; then
        has_current_branch_change=1
    fi
    ...
    if [[ $has_current_branch_change -eq 0 ]] && [[ -n "$tech_stack" ]]; then
        new_change_entry="- $CURRENT_BRANCH: 新增 $tech_stack"
    elif [[ $has_current_branch_change -eq 0 ]] && [[ -n "$NEW_DB" ]] && [[ "$NEW_DB" != "N/A" ]] && [[ "$NEW_DB" != "NEEDS CLARIFICATION" ]]; then
    ...
    if grep -Eq "^## (Active Technologies|当前使用技术)$" "$target_file" 2>/dev/null; then
        has_active_technologies=1
    fi
    
    if grep -Eq "^## (Recent Changes|最近变更)$" "$target_file" 2>/dev/null; then
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        if [[ "$line" == "## Active Technologies" || "$line" == "## 当前使用技术" ]]; then
            echo "$line" >> "$temp_file"
        ...
        # Update timestamp
        if [[ "$line" =~ (\*\*)?Last\ updated(\*\*)?:.*[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9] ]] \
            || [[ "$line" =~ 最后更新：[[:space:]]*[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9] ]]; then
