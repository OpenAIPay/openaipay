 ...
 * @date: 2026/03/04
 */
public class FundFastRedeemQuota {
    ...
    public FundFastRedeemQuota(String fundCode,
                               LocalDate quotaDate,
     ...
     * 初始化业务数据。
     */
    public static FundFastRedeemQuota init(String fundCode, LocalDate quotaDate, FundAmount quotaLimit, LocalDateTime now) {
    ...
    public void occupy(FundAmount amount, LocalDateTime now) {
        ...
        if (quotaUsed.add(value).compareTo(quotaLimit) > 0) {
            throw new IllegalArgumentException("fund fast redeem quota exceeded");
        }
