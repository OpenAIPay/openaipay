...
public class TradeSplitPlan {
    ...
    private TradeSplitPlan(CurrencyUnit currencyUnit,
     ...
     * 处理OF信息。
     */
    public static TradeSplitPlan of(CurrencyUnit currencyUnit,
                                    Money walletDebitAmount,
                                    Money fundDebitAmount,
                                    Money creditDebitAmount,
                                    Money inboundDebitAmount) {
        return new TradeSplitPlan(currencyUnit, walletDebitAmount, fundDebitAmount, creditDebitAmount, inboundDebitAmount);
    }

    /**
     * 处理业务数据。
     */
    public static TradeSplitPlan empty(CurrencyUnit currencyUnit) {
        return new TradeSplitPlan(currencyUnit, null, null, null, null);
    }
     ...
     * 处理业务数据。
     */
    public static TradeSplitPlan fromPayload(String payload) {
        if (payload == null || payload.isBlank()) {
            throw new IllegalArgumentException("trade split plan payload must not be blank");
        }
    ...
    private static Money normalizeNonNegative(Money amount, String fieldName, CurrencyUnit currencyUnit) {
        ...
        if (!currencyUnit.equals(amount.getCurrencyUnit())) {
            throw new IllegalArgumentException(fieldName + " currency must equal trade split plan currency");
        }
