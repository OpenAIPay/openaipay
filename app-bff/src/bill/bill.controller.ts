...
export class BillController {
  ...
  async getUserBillEntries(
    ...
    return this.billService.getUserBillEntries(
      normalizedUserId,
      this.normalizePageNo(pageNo),
      this.resolvePageSize(limit, pageSize),
      normalizedBillMonth,
  ...
  private normalizePageNo(value: unknown): number {
    if (value === undefined || value === null || value === '') {
      return 1;
    }
  ...
  private resolvePageSize(limitValue: unknown, pageSizeValue: unknown): number {
    if (pageSizeValue !== undefined && pageSizeValue !== null && pageSizeValue !== '') {
      return this.normalizePageSize(pageSizeValue);
    }
    return this.normalizeLimit(limitValue);
  }
